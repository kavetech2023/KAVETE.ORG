...
const Edit = () => {
    ...
    return (
        ...
        <div className="mt-10 w-full  items-center justify-start gap-2 flex rounded-lg bottom-0"></div>
        <div className="mt-5 grid items-center grid-cols-1 gap-3  w-full">
          <div className="grid grid-cols-1 sm:grid-cols-3  gap-3 bg-white rounded-lg shadow-lg p-6 h-full">
          <div style={{display:"flex",flexDirection:"row",gap:"20px"}}>
          <button onClick={() => { handleMoreInfo(); open(); }} > Generate Cv</button>
          </div>
          <div>
            <button>Generate Cover Letter</button>
          </div>
          <div>
          <button onClick={handlePrintRTP}> Download</button>
          </div>
