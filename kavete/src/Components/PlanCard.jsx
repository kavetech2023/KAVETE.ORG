...
const PlanCard = ({price, title, description, features,isPopular}) => {
  return (
    ...
    {
        isPopular && (<div className='popular-tag absolute -top-6 left-0 bg-indigo-500 text-white px-2 py-1 rounded-t-lg w-full text-center'>Popular</div>
        )
    }
        <div>
                    ...
                    features.map((feature, index) => (
                        <li key={index} className='flex flex-row items-center text-gray-700 gap-2 my-2'>
                            <div className='w-4 h-4 bg-indigo-500 rounded-full flex items-center justify-center'>
                                <Check size={10} className='inline-block  text-white w-3 h-3'/>
                                </div>
                                <p>{feature}</p>
                        </li>
                    ))
        ...
        <div>
            <button className='bg-indigo-500 text-white rounded-lg px-4 py-2 mt-4'>Select Plan</button>
        </div>
    </div>
